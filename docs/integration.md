...
| Nouvelles erreurs après passage en 0.3 | Depuis la **v0.3.0**, le codegen par défaut est celui de Volar (`@vue/language-core` du `vue-tsc` du projet) : vue-go-tsc remonte maintenant **les mêmes erreurs que vue-tsc**, y compris celles que l'ancien codegen Go ne voyait pas (ex. `aria-label` non converti en `ariaLabel` sur un composant qui exige la prop). Ce sont de vraies erreurs vue-tsc : les corriger. En dépannage, l'ancien comportement reste disponible : `vue-go-tsc --codegen=go …` (ou `VUE_GO_TSC_CODEGEN=go`). |
| `Volar codegen: cannot resolve "typescript"` | Le codegen Volar utilise le `typescript` du projet : l'installer en devDependency (`pnpm add -D typescript`), ou utiliser `--codegen=go`. |
