...
// toServicePosition converts an LSP position to a service code byte offset,
// applying the Golar forward mapping for framework files (e.g. .vue).
func (l *LanguageService) toServicePosition(file *ast.SourceFile, position lsproto.Position) int {
	pos := int(l.converters.LineAndCharacterToPosition(file, position))
	if callbacks := l.host.GetGolarCallbacks(); callbacks != nil && callbacks.PositionToService != nil {
		pos = callbacks.PositionToService(file, pos)
	}
	return pos
}
