...
func (l *LanguageService) getMappedLocation(fileName string, fileRange core.TextRange) lsproto.Location {
	...
	if startPos == nil {
		// For framework files (e.g. .vue), map service code positions back to source.
		if callbacks := l.host.GetGolarCallbacks(); callbacks != nil && callbacks.PositionFromService != nil {
			if program := l.GetProgram(); program != nil {
				if file := program.GetSourceFile(fileName); file != nil && file.GolarLanguageData != nil {
					mappedStart := callbacks.PositionFromService(file, fileRange.Pos())
					mappedEnd := callbacks.PositionFromService(file, fileRange.End())
					newRange := core.NewTextRange(mappedStart, mappedEnd)
					lspRange := l.createLspRangeFromRange(newRange, l.getScript(fileName))
