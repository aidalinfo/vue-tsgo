...
import (
	...
	"github.com/NikhilVerma/vue-tsgo/internal/pluginhost"
...
func init() {
	...
	// Always register .vue as a supported extension so the file loader
	// includes .vue files in the program. Without this, the filesparser
	// rejects .vue files when AllowNonTsExtensions is not set.
...
func init() {
	...
	for pluginName := range strings.SplitSeq(pluginNames, ",") {
		...
		case "svelte":
			sveltePlugin, err = pluginhost.NewPlugin([]string{"node", "packages/svelte/src/index.ts"})
			if err != nil {
				panic(err)
			}
		...
		case "astro":
			astroPlugin, err = pluginhost.NewPlugin([]string{"packages/astro/astro"})
			if err != nil {
				panic(err)
			}
...
func parseFile(fs vfs.FS, opts ast.SourceFileParseOptions, sourceText string, scriptKind core.ScriptKind) *ast.SourceFile {
	// .vue files from disk get ScriptKindUnknown; default to TS
	if scriptKind == core.ScriptKindUnknown && (strings.HasSuffix(opts.FileName, ".vue") || strings.HasSuffix(opts.FileName, ".svelte") || strings.HasSuffix(opts.FileName, ".astro")) {
...
func positionToService(file *ast.SourceFile, pos int) int {
	...
	langData := file.GolarLanguageData.(languageData)
	results := langData.sourceMap.ToServiceLocation(uint32(pos))
	if len(results) == 0 {
		return pos
	}
	// Prefer non-zero-length mappings (actual content) over zero-length
	// point mappings (tag markers). Zero-length mappings at the same offset
	// would map to incorrect positions like __VLS_ctx prefix instead of
	// the actual identifier.
	for _, loc := range results {
		if loc.Mapping.SourceLength > 0 {
			return int(loc.Offset)
		}
	}
	return int(results[0].Offset)
}

// positionFromService maps a service code position back to the source position.
func positionFromService(file *ast.SourceFile, pos int) int {
	if file.GolarLanguageData == nil {
		return pos
	}

	langData := file.GolarLanguageData.(languageData)
	results := langData.sourceMap.ToSourceLocation(uint32(pos))
	if len(results) == 0 {
		return pos
	}
	// Prefer non-zero-length mappings (actual content) over zero-length
	// point mappings (tag markers).
	for _, loc := range results {
		if loc.Mapping.SourceLength > 0 {
			return int(loc.Offset)
		}
...
var GolarExtCallbacks = &golarext.GolarCallbacks{
	...
	PositionFromService: positionFromService,
